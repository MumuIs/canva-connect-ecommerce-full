  ...
  const fetchAndExportUpdatedDesign = async () => {
    try {
      ...
      switch (parsedCorrelationState.originPage) {
        ...
        /**
         * Case 4 - In the case of originally starting from brand template creator page:
         *  4a) Handle the newly created design from brand template
         *  4b) Show success message with design details
         *  4c) Optionally save the design information for future reference
         */
        case EditInCanvaPageOrigins.BRAND_TEMPLATE_CREATOR: {
          // 获取设计详情
          const designDetails = {
            id: fetchDesignResult.design.id,
            title: fetchDesignResult.design.title || "Brand Template Design",
            editUrl: fetchDesignResult.design.urls?.edit_url,
            viewUrl: fetchDesignResult.design.urls?.view_url,
            thumbnail: fetchDesignResult.design.thumbnail,
            createdAt: new Date().toISOString(),
          };

          // 显示成功消息，包含设计详情
          addAlert({
            title: `成功创建了设计副本: ${designDetails.title}!`,
            body: `设计 ID: ${designDetails.id}`,
            variant: "success",
            hideAfterMs: 8000,
          });

          // 可选：将设计信息保存到本地存储或发送到服务器
          console.log("Brand Template 创建的设计详情:", designDetails);
          
          // 可选：显示设计链接
          if (designDetails.editUrl) {
            setTimeout(() => {
              addAlert({
                title: "设计已创建完成",
                body: `你可以继续编辑这个设计`,
